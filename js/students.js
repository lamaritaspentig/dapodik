...
async function loadData(fromLogin=false){
  ...
  try{
    ...
    if(!res || res.ok===false) throw new Error(res?.message||'Data awal belum dapat dimuat.');
    ...
    if(state.auth.user?.role==='SISWA'){
      ...
      if(own){
        ...
        // Jangan tampilkan panel daftar kosong untuk role siswa.
    ...
    state.activities=Array.isArray(state.dashboard.recentActivities)?state.dashboard.recentActivities:[];
    ...
    // Tidak lagi memuat konflik, aktivitas penuh, pengajuan dan backup saat login.
    // Semua menu tersebut bersifat lazy-load.
  }catch(err){
    setLoading(false);
...
async function loadStudentPage(silent=false){
  ...
  try{
    ...
    if(Array.isArray(res.classes) && res.classes.length){
...
function applyPendingRequestIndicators(student){
  ...
  Object.entries(changes).forEach(([key,val])=>{
    ...
    wrap.insertAdjacentHTML('beforeend',`<div class="pending-field-note"><i class="bi bi-arrow-right-circle"></i>Nilai diajukan: <strong>${esc(String(val||'(kosong)'))}</strong></div>`);
  });
}
