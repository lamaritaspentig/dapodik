...
async function doLogin(e){
  ...
  try{
    ...
    setLoginMessage('Berhasil masuk. Menyiapkan halaman...','success');
    showApp();
    await loadData(true);
  }catch(err){
...
async function doLogout(){
  try{ if(state.auth.token) await apiPost('logout',{},state.auth.token); }catch(_){ }
  clearSession();
  showLogin();
}
...
function applyRoleUI(){
  ...
  if(isStudent) showView('students');
}
...
function applyApiInfo(){
  ...
  if(!configured){
    ...
    setLoginMessage('Isi DEPLOYMENT_ID di js/config.js menggunakan ID dari URL Web App /exec.','warning');
    return;
  }

  updateLoginApiStatus('checking','Memeriksa deployment GAS...');

  apiGet('ping')
    .then(ping=>{
      if(!ping?.ok){
        throw new Error(ping?.message || 'Endpoint ping tidak merespons.');
      }

      if(!ping?.version){
        updateLoginApiStatus('offline','Deployment GAS masih versi lama');
        updateDbStatus(false,'Perlu deploy ulang');
        setLoginMessage('Code.gs pada Web App belum versi terbaru. Deploy ulang sebagai versi baru, lalu gunakan URL /exec yang sama.','warning');
        return null;
      }

      if(String(ping.version)!==String(APP.VERSION)){
        updateLoginApiStatus('offline','Versi GAS tidak sesuai');
        updateDbStatus(false,'Versi berbeda');
        setLoginMessage(`Deployment GAS terdeteksi versi ${ping.version}. Versi yang dibutuhkan adalah ${APP.VERSION}. Deploy Code.gs yang sesuai sebagai versi baru.`,'warning');
        return null;
      }

      updateLoginApiStatus('checking','Memeriksa database...');
    ...
    .then(res=>{
      ...
      if(res?.ok && res?.ready){
        updateLoginApiStatus('online','Sistem siap digunakan');
        updateDbStatus(true,'Online');
        setLoginMessage('','');
        return;
      }

      if(res?.ok && res?.status==='SETUP_REQUIRED'){
        updateLoginApiStatus('offline','Database belum disiapkan');
        updateDbStatus(false,'Belum siap');
        setLoginMessage('Jalankan setupDatabase() dan setupAdminPertama() dari editor Apps Script, kemudian deploy ulang Web App.','warning');
        return;
      }

      if(res?.ok && res?.status==='DATABASE_UNAVAILABLE'){
        updateLoginApiStatus('offline','Spreadsheet tidak dapat diakses');
        updateDbStatus(false,'Database gagal');
        setLoginMessage('Apps Script tidak dapat membuka Spreadsheet database. Jalankan kembali setupDatabase() dari project Apps Script yang terikat ke Spreadsheet.','danger');
        return;
      }

      updateLoginApiStatus('offline','Sistem belum siap');
      updateDbStatus(false,'Belum siap');
      setLoginMessage(res?.message || 'Respons API tidak sesuai dengan versi aplikasi.','warning');
    })
    .catch(err=>{
      console.error('Pemeriksaan API gagal:',err);
      updateLoginApiStatus('offline','Tidak dapat mengakses Web App');
      updateDbStatus(false,'Offline');
      setLoginMessage(
        'Web App GAS tidak dapat diakses. Pastikan deployment: Execute as = Me dan Who has access = Anyone, lalu buat deployment versi baru.',
        'danger'
      );
    });
}
...
function updateLoginApiStatus(stateName,text){
  ...
  if(stateName==='offline') el.classList.add('offline');
