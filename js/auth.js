...
async function doLogin(e){
  ...
  try{
    ...
    if(res.deferredBootstrap && res.user?.role==='ADMIN'){
      // Shell Admin sudah tampil. Data awal dimuat tanpa overlay yang menahan UI.
      loadData({background:true});
    }else{
      applyBootstrapResponse(res);
    }

    if(Number.isFinite(Number(res.serverMs))){
      console.info('Login server:',Number(res.serverMs)+' ms');
    }
  }catch(err){
...
function clearSession(){
  ...
  // Naikkan sequence agar response halaman siswa yang sedang berjalan menjadi usang.
  state.studentPageRequestSeq=(Number(state.studentPageRequestSeq)||0)+1;
  ...
  state.religionMode='kelas';
  state.tableFull=false;
  state.selectedStudentActionId='';

  // Hancurkan chart lama agar data akun sebelumnya tidak sempat tampil.
  ...
  resetValue('searchInput','');
  resetValue('classFilter','');
  resetValue('genderFilter','');
  resetValue('pageSize','25');
  resetValue('requestStatusFilter','');
  resetValue('activitySearch','');
  resetValue('activityActorFilter','');
  resetValue('activityDateFrom','');
  resetValue('activityDateTo','');

  // Bersihkan form dan indikator pengajuan siswa lama.
  const studentForm=document.getElementById('studentForm');
  if(studentForm) studentForm.reset();

  document.querySelectorAll('.pending-field-badge,.pending-field-note').forEach(el=>el.remove());
  ...
  // Kosongkan tampilan data privat agar tidak berkedip saat akun lain masuk.
  ['statTotal','statL','statP','statRombel','statPendingRequests',
   'statConflicts','statIncomplete','statTodayActivities'].forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.textContent='0';
  });

  const privateContainers=[
    'dashboardActivityList','activityList','requestTableBody','conflictTableBody',
    'studentTableBody','genderTableBody','genderRombelTableBody',
    'religionTableBody','religionRombelTableBody'
  ];
  privateContainers.forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.innerHTML='';
  });

  [
    'dashboardGenderChart','dashboardReligionChart',
    'genderChart','genderRombelChart',
    'religionChart','religionRombelChart'
  ].forEach(id=>{
    const el=document.getElementById(id);
    if(el) el.innerHTML='';
  });
...
function showApp(){
  ...
  // Jangan mewarisi panel form/list dari akun sebelumnya.
  if(isStudent){
